(function() {
        ...
        return {
            ...
            link: function($scope, element, attrs, modelCtrl) {
                ...
                _formatted = function(viewVal) {
                    ...
                    if(opts.singleDatePicker) return f(viewVal.startDate);
    ...
    picker.directive('daterangepicker', function($compile, $timeout, $parse, dateRangePickerConfig) {
        return {
            ...
            link: function($scope, element, attrs, modelCtrl) {
                var el, opts, _init, _picker,_formatter;
                el = $(element);
                ...
                _formatter = function(val){
                    var formattedVal;
                    if (!_picker.singleDatePicker) {
                        formattedVal = val.startDate.format(_picker.format) + _picker.separator + val.endDate.format(_picker.format);
                    } else{
                        formattedVal = val.endDate.format(_picker.format);
                    }
                    return formattedVal;
                }
                ...
                modelCtrl.$formatters.push(function(val) {
                    if (val && val.startDate && val.endDate) {
                        ...
                        var formattedVal = _formatter(val);
                        return formattedVal;
                    }
                    return '';
                });
                ...
                _init = function() {
                    el.daterangepicker(opts, function(start, end, label) {
                        return $timeout(function() {
                            return $scope.$apply(function() {
                                modelCtrl.$setViewValue(_formatter({
                                    startDate: start,
                                    endDate: end
                                }));
                            });
                ...
                if (attrs.options) {
                    $scope.$watch('opts', function(newOpts,oldOpts) {
                        if(newOpts == oldOpts) return;
                        opts = angular.extend({},dateRangePickerConfig, newOpts);
                        _init();
                    });
                }
            }
